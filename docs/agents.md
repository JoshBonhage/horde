...
# Patterns proving this agent's UI is on screen. Presence detection works even when the
# process name is unhelpful — a wrapper script, an interpreter, a versioned binary.
detect = ['esc to interrupt', 'shift\+tab to cycle', '⏵⏵']
