...
struct PtyContent {
    ...
    ///
    /// Shared with the emulator's event proxy, which runs on the reader thread. Here rather
    /// than on `Pane` because only a running program can ask: a doc pane has nobody to tell.
    facts: Arc<PaneFacts>,
...
impl Pane {
    ...
    pub fn resize(&mut self, cols: u16, rows: u16) -> Result<()> {
        ...
        match &mut self.content {
            Content::Pty(p) => {
                // Before the SIGWINCH: a program that answers the signal by asking how big it
                // now is must not be told the size it had a moment ago.
                p.facts.set_size(cols, rows);
